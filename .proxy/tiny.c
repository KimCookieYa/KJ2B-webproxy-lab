...
void doit(int fd) {
    ...
    if (strcasecmp(method, "GET")) {                // 요청 메소드가 GET이 아니면 에러 처리
    ...
    if (stat(filename, &sbuf) < 0) {                // 파일이 디스크에 없으면 에러 처리
        clienterror(fd, filename, "404", "Not found", "Tiny couldn't find this file");
        return;
    }

    if (is_static) {                                                  // 정적 컨텐츠인 경우
        if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) {  // 일반 파일이 아니거나 읽기 권한이 없는 경우 에러 처리
            clienterror(fd, filename, "403", "Forbidden", "Tiny couldn't read the file");
            return;
        }
        serve_static(fd, filename, sbuf.st_size, method);             // 정적 컨텐츠 제공
    } else {                                                          // 동적 컨텐츠인 경우
        if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) {  // 일반 파일이 아니거나 실행 권한이 없는 경우 에러 처리
...
int parse_uri(char *uri, char *filename, char *cgiargs) {
    ...
    if (!strstr(uri, "cgi-bin")) {  // uri에 cgi-bin이 없으면 정적 컨텐츠
        strcpy(cgiargs, "");
